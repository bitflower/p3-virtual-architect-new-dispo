...
| Knob | Points at | Default if unset |
|---|---|---|
| `PRIOR_ART_SOURCES` | Glob(s) for pattern-mining / competitor / comparison docs | skip layer, flag gap in audit |
| `DIRECTION_SOURCES` | Vision / roadmap / strategy docs | skip layer, flag gap |
| `EXISTING_WORK_SOURCES` | In-flight / planned features (collision + numbering) | skip collision check, flag |
| `CONVENTIONS_FILE` | Binding conventions doc(s), e.g. `CLAUDE.md` (root + nested) | none — infer from repo |
| `FEATURE_ID_SCHEME` | How features are identified (e.g. `NNN_<slug>`, id = max+1) | `<slug>` only |
| `OUTPUT_PATH_TEMPLATE` | Where the PRD file is written | `docs/prd/<id>.md` |
| `REFERENCE_PRD_SURFACE` | Example PRD to mirror for backend/API features | none |
| `REFERENCE_PRD_UI` | Example PRD to mirror for UI features | none |
| `SEARCH_AGENT` | Read-only search agent for deep reads | `general-purpose`, or read inline |
