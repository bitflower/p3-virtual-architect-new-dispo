...
Three failure scenarios threaten data consistency between New Dispo and TMS during transport order creation. Each scenario has different characteristics, impacts, and recovery requirements.

**See detailed analysis:** [TMS Sync Failure Scenarios](./tms-sync-failure-scenarios.md)

### Quick Reference

| Scenario | Type | Impact | Recovery Needed |
|----------|------|--------|-----------------|
| **1. Early Failure from Bridge** | Consistent State | No data written, clean failure | None - user can retry |
| **2. Local DB Failure Post-TMS** | Data Out of Sync | TMS updated, New Dispo not | State checking + reconciliation |
| **3. Network Interruption** | Uncertain State | TMS updated, New Dispo unknown | State query + reconciliation |

**Key Points:**
- **Scenario 1:** TMS Bridge returns 4xx/5xx error before TMS commits. No inconsistency, safe to abort.
- **Scenario 2:** TMS succeeds but New Dispo DB unavailable. Requires reconciliation.
- **Scenario 3:** TMS succeeds but response lost in network. Requires state query before retry to avoid duplicates.

For complete sequence diagrams, characteristics, and examples, see: [TMS Sync Failure Scenarios](./tms-sync-failure-scenarios.md)
...
**Not required for Scenario 1:** If TMS Bridge returns error early (400/500), no state written anywhere. Safe to abort, no recovery mechanism needed.

**Required for Scenario 2:** If TMS succeeds but New Dispo DB unavailable, outbox ensures eventual alignment. This is primary use case justifying outbox pattern.
...
**Implementation:**
- On sync failure (Scenarios 2, 3), rollback local transaction or mark as "pending"
...
- Failures are rare in stable infrastructure (Scenarios 2, 3)
- Scenario 1 (early fail) already handled gracefully
...
|-----------|----------|--------------|----------|
| TMS Bridge 4xx | 1 | No | Immediate user error |
| TMS Bridge 5xx | 1 | Yes (transient) | Retry or user-driven |
| New Dispo DB outage | 2 | Yes (transient) | Retry or user-driven |
| Network timeout | 3 | Uncertain | Reconciliation required |
| TMS constraint violation | 1 | No | User must fix data |
