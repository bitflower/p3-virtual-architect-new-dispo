# 1. Change Contractor (TMS as source)

| **User Stories**                                                                                                                       |
| -------------------------------------------------------------------------------------------------------------------------------------- |
| [119730: Edit Flow – 2.1 Change Contractor (TMS as source)](https://dev.azure.com/p3ds/Nagel-CAL%20Disposition/_workitems/edit/119730) |

## Management Summary

This feature allows dispatchers to change the contractor ("Unternehmer") on a Transport Order. The implementation reuses the existing TMS core function `pTA.addUnt` via a new wrapper `pDis_TransportOrder.SetContractor`.

**Key decisions:**
- Reuse existing `pTA.addUnt` instead of custom implementation
- All standard side effects are triggered (truck/trailer removal, billing recalculation, status updates)
- Contractor lookup uses the `person` master data table
- Mode can be `UNF` (Freight Contractor) or `UNN` (NV Contractor) – business decision pending on which one to use
...
sequenceDiagram
    ...
    User->>FE: Type in contractor name field (e.g. "Helmut Log")
    activate FE

    loop On each keystroke
        FE->>BE: searchContractor(searchTerm)
        activate BE
...
When no candidate is selected from the contractor lookup, the user can manually input address fields. To ensure data consistency with TMS master data, the combination of **Country**, **ZIP code**, and **City** must be validated against the `ort` table in the TMS database before the contractor can be saved.
...
:::mermaid
flowchart TD
    A[User enters manual contractor data] --> B{All required fields populated?}
...
## 2. Store Contractor in Transport Order

Flow for setting or updating the contractor ("Unternehmer") on a Transport Order using the new wrapper function `pDis_TransportOrder.SetContractor`. This wrapper calls the TMS core function `pTA.addUnt` which handles writing to `pers` and manages the relationship to the Transport Order via `sen_tb`. The procedure handles both inserting a new contractor and updating an existing one.

**Mode:** For this flow, `nMode` is either:
...
sequenceDiagram
    ...
    UI->>FE: Save contractor
    FE->>BE: SetContractor(nTATix, nPersTix, nMode)
    BE->>Bridge: GraphQL: setContractor mutation
    Bridge->>TMS: pDis_TransportOrder.SetContractor(nTATix, nPersTix, nMode)

    Note over TMS: nPersTix from person lookup or new contact

    rect rgb(230, 245, 230)
        Note over TMS: Inside TMS Database
        TMS->>TMS: pTA.addUnt(nTATix, sPersTB, nPersTix, nMode)
        Note over TMS: Derives sPersTB and PERS record from nPersTix
        TMS->>TMS: pPers.ADD(rPers) → INSERT/UPDATE pers
        TMS->>TMS: UPDATE/INSERT sen_tb
        TMS->>TMS: PTA.REMLKW (remove truck)
        TMS->>TMS: PTA.REMANH (remove trailer)
        TMS->>TMS: DELETE LST_B, LST_K (billing)
        TMS->>TMS: PTA.SETSENLSTB (set billing list)
        TMS->>TMS: UPDATE/INSERT sen_frk_unt
        TMS->>TMS: PTA.SETSTATUS (update status)
    end
...
### Steps (using pDis_TransportOrder.SetContractor)

1. Receive `nTATix` (Transport Order TIX), `nPersTix` (person TIX), and `nMode` (`UNF` or `UNN`) from UI/API
2. Call `pDis_TransportOrder.SetContractor(nTATix, nPersTix, nMode)` which:
   - Derives `sPersTB` and `PERS` record from `nPersTix`
   - Passes `nMode` through to `pTA.addUnt`
   - Calls `pTA.addUnt(nTATix, sPersTB, nPersTix, nMode)` internally
3. `pTA.addUnt` handles:
   - Creating/updating `pers` record via `pPers.ADD`
   - Creating/updating `sen_tb` link (Transport Order + Contact Type + Person)
   - Removing truck assignment (`PTA.REMLKW`)
   - Removing trailer assignment (`PTA.REMANH`)
   - Deleting billing entries (`LST_B`, `LST_K` - Unternehmerpauschalen)
   - Setting billing list (`PTA.SETSENLSTB`)
   - Updating `sen_frk_unt` (contractor link for freight)
   - Updating Transport Order status (`PTA.SETSTATUS`)
...
erDiagram
    ...
    SENDUNG ||--o{ SEN_FRK_UNT : "has contractor"
    SEN_TB ||--|| PERS : "references"
    SEN_FRK_UNT ||--|| PERS : "references"
...
### pDis_TransportOrder.SetContractor (Draft)

```sql
PROCEDURE SetContractor(
    nTATix   IN NUMBER,   -- Transport Order TIX (sendung.tix)
    nPersTix IN NUMBER,   -- Person TIX from person table (NULL for manual input)
    nMode    IN NUMBER,   -- 1 = UNF (Freight), 2 = UNN (NV)
...
BEGIN
    -- Determine TB (contact type) based on mode
    IF nMode = 1 THEN
        sPersTB := 'UNF';  -- Unternehmer Fracht
    ELSIF nMode = 2 THEN
        sPersTB := 'UNN';  -- Unternehmer NV
    ELSE
        RAISE_APPLICATION_ERROR(-20001, 'Invalid mode: ' || nMode);
    END IF;
...
**Notes:**
- When `nPersTix` is provided, the procedure looks up the contractor from `person` master data
- When `nPersTix` is NULL, the manual input fields are used (location validation must be done before calling)
- The procedure delegates all side effects (truck/trailer removal, billing, status updates) to `pTA.addUnt`

- **Core procedure** `pTA.addUnt` signatures:

```sql
pTA.addUnt(nTATix in number, sPersTB in varchar2, nPersTix in number, nMode in number)
pTA.addUnt(nTATix in number, sPersTB in varchar2, rUnt in pers%rowtype, nMode in number)
```

- Source: `src/sql/package/PTA.sql` (lines ~3050-3205)

![addUnt procedure](addUnt.png)
