# 3. Vehicle Properties & Body Type
...
cal_uniface.item('TempLog_B', p.properties_raw) AS temp_recorder_required,
...
ELSE coalesce(cal_uniface.item('TempLog_B', v_current), 'F') END,
...
| UI Label (DE) | View/API Column | Internal Key | Example Value |
| -------------------------------- | ----------------------- | ------------ | ------------- |
| Temperaturschreiber erforderlich | `temp_recorder_required`| `TempLog_B`  | `TempLog_B=T` |
...
```
vorkuehl_b=T doppelstock_b=T trennwand_b=0 atp_frc_b=T atp_frb_b=1 wb_b=T plane_b=F tank_b=F TempLog_B=T atp_koffer_b=F
```
