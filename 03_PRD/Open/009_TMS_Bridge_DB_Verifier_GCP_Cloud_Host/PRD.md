...
- **M8: `MarkdownReporter` in `.Core`.** Generates a markdown status report from a `VerificationResult`. Pure code formatting — no AI/LLM involved. Same pattern as the existing `ConsoleReporter` and `JsonReporter`. Produces the rich report format used in `reports/`: header, summary table, full per-object check matrix (per-level ✅/❌/—), verification-levels severity legend, violations grouped by level (L1–L5), and an appendix (column type mismatches, schema drift, deprecated objects).
