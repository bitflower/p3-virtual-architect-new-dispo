...
xServer Integration for Tour Optimization in New Disposition System
...
2.  **Generating PoolDTO** using existing `pTop_LoadingList.get()` stored procedure.
3.  **Calling xServer** for route calculation using TOP DLL functionality.
4.  **Processing Results** and storing optimized tour information.
5.  **Maintaining Compatibility** with existing TMS processes and data structures.
6.  **Enabling P3 Independence** while reusing CAL's domain expertise.
7.  **Supporting Future Migration** to a cleaner architecture in subsequent iterations.
