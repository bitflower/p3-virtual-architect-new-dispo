...
| Knob | What it is | Value |
|---|---|---|
| `REVIEWER_ARCHITECTURAL` | Agent for architecture/security review | `senior-code-reviewer` (local agent in `.claude/agents/`) |
| `REVIEWER_CLEANCODE` | Agent for clean-code/cohesion review | `senior-clean-code-reviewer` (local agent in `.claude/agents/`) |
| `TEST_CMD` | Full test-suite command | **Backend:** `dotnet test Code/Disposition-Backend/CALConsult.Disposition.API.sln` · **TMS Bridge:** `dotnet test Code/Disposition-Abstraction-Layer/*.sln` · **Frontend:** `cd Code/Disposition-Frontend && npx nx run-many --target=test` |
| `START_CMD` | How to start the stack for smoke test | `/local-dev-start` skill (runs TMS Bridge, Backend, Frontend via local-dev skills) |
| `BRANCH_SCHEME` | Feature branch naming | `feature/<slug>` |
| `PLAN_FILENAME` | Implementation-plan filename | `IMPLEMENTATION_PLAN.md` |
| `CONVENTIONS_FILE` | Binding conventions doc(s) | `CLAUDE.md` (root) + nested CLAUDE.md in component repos if present |
| `STACK_NOTES` | Project idioms for typed contracts / components / db | **Backend:** .NET 8 CQRS (MediatR), EF Core, FluentValidation, MSTest, C# 12 primary constructors — no service/repository layer · **TMS Bridge:** .NET 8 HotChocolate GraphQL, multi-tenant via `databaseIdentifier`, `IRoutineExecutor` + `RoutineParameterBuilder` for stored procs · **Frontend:** Angular 19 standalone components, Angular Material, Nx monorepo, `RequestService` (not `HttpClient`), `BehaviorSubject` for state · **DB:** AlloyDB (PostgreSQL) + Oracle (legacy TMS) |
