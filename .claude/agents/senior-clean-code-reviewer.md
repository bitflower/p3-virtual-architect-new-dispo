...
- Backend follows CQRS: commands, queries, handlers — no service/repository layer.
- C# 12 primary constructors for DI.
- TMS Bridge is multi-tenant: every query requires a `databaseIdentifier`.
- Frontend: one standalone component per file, `RequestService` (not raw `HttpClient`), `BehaviorSubject` for state.
- Angular Material is the UI component library / design system.
...
### Design-system rules (Angular Material — Frontend only)

The Frontend uses Angular Material with a custom theme. Read tokens from the theme file in the
Frontend repo (`Code/Disposition-Frontend`).
